...
export class Button extends Disposable {
	...
	constructor(container: HTMLElement, options?: IButtonOptions) {
		...
		DOM.addClass(this._element, styles['monaco-button']);
		...
		this._register(DOM.addDisposableListener(this._element, DOM.EventType.MOUSE_OVER, e => {
			if (!DOM.hasClass(this._element, styles['disabled'])) {
	...
	set label(value: string) {
		if (!DOM.hasClass(this._element, styles['monaco-text-button'])) {
			DOM.addClass(this._element, styles['monaco-text-button']);
		}
	...
	set enabled(value: boolean) {
		if (value) {
			DOM.removeClass(this._element, styles['disabled']);
			this._element.setAttribute('aria-disabled', String(false));
			this._element.tabIndex = 0;
		} else {
			DOM.addClass(this._element, styles['disabled']);
	...
	get enabled() {
		return !DOM.hasClass(this._element, styles['disabled']);
	}
